...
// Big O: BEST CASE O(n), WORST/AVERAGE CASE O(n^2)

function bubbleSort(arr) {
	let swapped = true;
  // Set to true initially to start the while loop
  // Track if values or elements in the array have switched
	while (swapped) {
		swapped = false;
    // set to false initially
    // Exits the while loop if nothing has swapped
		for (let j = 0; j < arr.length; j++) {
      // Loop over the whole array to compare values
			if (arr[j] > arr[j + 1]) {
        // If the element on the left is greater than the element on the right (just one higher)
				let temp = arr[j];
        // set a temporary value before swapping
				arr[j] = arr[j + 1];
        // swap the values of arr[j] and arr[j+1]
				arr[j + 1] = temp;
        // Taking the the temporary value and putting it back where the swap happened
				swapped = true;
        // Since a swap happened, we set swapped back to true and the while loop repeats
			}
		}
	} 
	return arr;
...
function bubblesSort(array) {
  // Bubble sort with index tracking instead of using a swapped boolean
  let n = array.length;
  while (n > 0) {
...
console.log(bubblesSort(numbers))