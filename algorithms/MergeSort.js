...
function mergeSort(arr) {
  ...
  // Set a midpoint to split the array in half
  let left = arr.slice(0, mid);
  // Use the midpoint to create a left array
  let right = arr.slice(mid, arr.length);
  // Use the midpoint to create a right array
  ...
  // Return both sorted arrays mergec together
}

// Helper function (merge)
function merge(arr1, arr2) {
  // Takes in both arrays, left and right
  let sort = [];
  // Create an empty array to store sorted values
  
  
  while (arr1.length > 0 && arr2.length > 0) {
  // While BOTH arrays have length (at least one item)
    if (arr1[0] > arr2[0]) {
    // whichever array element is lesser, we'll push into the sort array
...
function mergeSort2(array, start, end) {
  ...
  if (start < end) {
    ...
    mergeSort2(array, start, middle) // sort first half - left array
    mergeSort2(array, middle + 1, end)  // sort second half - right array
...
// The merge() function typically gets 4 parameters: the complete array and the starting, middle, and ending index of the subarray. 
// The start, middle, and end index are used to create 2 subarrays, the first ranging from start to middle 
// and second ranging from middle to end. At this point, each subarray is in the correct order.
// The two subarrays are merged back together in order.
// merges two subarrays of array[]
function merge2(array, start, middle, end) {
  // create temp arrays
...
console.log(mergeSort2(numbers, 0, numbers.length - 1))