...
let nums = [1, 5, 2, 99, 22, 45436, 123, 3, 6, 98, 12, 56, 32]
// BIG-O
...
function quickSort(arr) {
	if (arr.length <= 1) {
    // BASE CASE
    // arr.length is less than or equal to 1
    // Arrays will split into left and right partitions until each array only has one item
    return arr;
  } 
  // Otherwise RECURSIVE CASE
  // console.log(arr)
  let leftArr = [];         
  // Create a left partition array     
  let rightArr = [];
  // create a right partition array
  let newArr = [];

  let pivot = arr.pop();      //  Take a pivot value
  // could be any value, so we'll pop() off the end of the array
  let length = arr.length;
  // Get the new array length after popping off the last item
  for (let i = 0; i < length; i++) {
    // Iterate through the whole array to compare all values against our pivot
    if (arr[i] <= pivot) {    // using pivot value start comparing
      // If an array value is less than or equal to the pivot value, we'll add it to the left array
      leftArr.push(arr[i]);      
    } 
    else {
      // Otherwise, we'll add the value to the right array if it's greater than our pivot
      rightArr.push(arr[i]);
    }
  }
  return newArr.concat(quickSort(leftArr), pivot, quickSort(rightArr)); 
  // Recursive return
  // returning the to arrays (left and right) joined together and continuing to sort                                              //returned untill sorting occurs
} 
...
function quickSortWithPartition(array, startIndex, endIndex) {
  // verify that the start and end index have not overlapped
  if (startIndex < endIndex) {
    // calculate the pivotIndex
    let pivotIndex = partition(array, startIndex, endIndex)
    // sort the left sub-array
    quickSortWithPartition(array, startIndex, pivotIndex)
    // sort the right sub-array
    quickSortWithPartition(array, pivotIndex + 1, endIndex)
  }
  return array;
}
...
// console.log(quickSortWithPartition(numbers, 0, numbers.length-1))